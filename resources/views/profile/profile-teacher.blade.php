...
<div class="container-fluid" id="container-wrapper">
    ...
    <div class="row mb-3">
                        ...
                        <div class="card-body">
                            @if (isset($profile->photo))
                                <img src="http://192.168.43.105:8000/{{ $profile->photo }}"
                                    class="card-img-top rounded-circle mb-2" style="max-width: 260px" alt="Photo">
                            @else
        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-header mb-0 d-flex flex-row align-items-center justify-content-between">
            ...
            $.ajax({
                ...
                error: function(response) {
                    if (response.responseJSON.success == false) {
                        ...
                        $(".preloader").fadeOut();
                        $('#name-error').text(response.responseJSON.name);
                        $('#username-error').text(response.responseJSON.username);
                        $('#email-error').text(response.responseJSON.email);
                        $('#address-error').text(response.responseJSON.address);
                        $('#birthplace-error').text(response.responseJSON.birthplace);
                        $('#birthdate-error').text(response.responseJSON.birthdate);
                        $('#gender-error').text(response.responseJSON.gender);
                        $('#phone-error').text(response.responseJSON.phone);
                        $('#religion-error').text(response.responseJSON.religion);
                        $('#photo-error').text(response.responseJSON.photo);
                    }
                },
            });
        });
